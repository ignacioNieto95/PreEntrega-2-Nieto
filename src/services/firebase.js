...
export async function crearOrdenCompra(ordenCompra){

    const collectionRef = collection(database, "ordenes")
    let respuesta = await addDoc(collectionRef, ordenCompra)
    return respuesta.id

}

async function mandarDataFirebase(data){
    ...
    let itemsCollectionRef = collection(database, "salsasdelproyecto")

    for(let item of dataSalsas){
        delete(item.id)
